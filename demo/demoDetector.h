...
{
public:
  KeyFrame(size_t id, double Timestamp, cv::Mat image): m_id(id),m_Timestamp(Timestamp),m_im(image) { }
 // void setRt(const Eigen::Matrix<double,3,4> Rt)
 // {
 //   m_Rt = Rt;
 // }
...
public:
  ...
  cv::Mat m_im;
  cv::Mat m_depth;
   vector<cv::KeyPoint> m_keys;
  vector<TDescriptor> m_descriptors;
  //Eigen::Matrix<double,3,4> m_Rt;
  ...
  (const std::string &name, const FeatureExtractor<TDescriptor> &extractor)
{
  ...
  //KeyFrames;
  typedef KeyFrame<TDetector> MyKeyFrame;
  std::vector<KeyFrame<TDetector> *> p_keyfames;
  ...
  for(unsigned int i = 0; i < filenames.size(); ++i)
  {
    ...
    // save keyframe;
    MyKeyFrame * newkeyfame = new MyKeyFrame(i, i, im );
    newkeyfame->setXY(xs[i], ys[i]);
    p_keyfames.push_back(newkeyfame);
  ...
  {
    cout << count << " loops found in txshis image sequence!" << endl;
  } 
  ...
  cout << endl << "Execution time:" << endl
    ...
    cout<<"the size of database: "<< detector.getDatabaseSize()<<std::endl;
